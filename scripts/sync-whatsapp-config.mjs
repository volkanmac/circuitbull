/**
 * Push WhatsApp Cloud API settings from .env → KV `config:whatsapp`.
 * Empty keys keep any token already in KV. Does not print secrets.
 */
import "dotenv/config";
import { pathToFileURL } from "node:url";
import { mergeWhatsAppKv, WHATSAPP_KV_KEY, whatsappPublicStatus } from "./lib/whatsapp-config.mjs";
...
export async function syncWhatsAppConfig({ putKey: put, getJson: get, nsId } = {}) {
  ...
  const existing = await getter(namespace, WHATSAPP_KV_KEY);
  const merged = mergeWhatsAppKv(process.env, existing);
  await setter(namespace, WHATSAPP_KV_KEY, merged);
  return { nsId: namespace, whatsapp: whatsappPublicStatus(merged) };
}

const isMain = process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href;
if (isMain) {
  const out = await syncWhatsAppConfig();
